...
| 8.2 | **Linking** ✅ | ★★ | Invoke linker hệ thống (`clang++` on Windows/POSIX, via `CreateProcessA`/`std::system`). Link libc mặc định (malloc/free/printf). Flag: `-o app.exe` tự động link. `__ivy_panic` runtime defined inline (puts+abort). 15/19 examples link+run successfully |
