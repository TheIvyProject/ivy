...
| 8.4 | **Cast operators** ✅ | ★★ | `static_cast<T>(expr)` (numeric/pointer/enum), `reinterpret_cast<T>(expr)` (unsafe only), `const_cast<T>(expr)` (unsafe only), C-style `(T)expr` (unsafe only). `dynamic_cast` cấm (no RTTI). Full pipeline: AST→HIR (type check + unsafe validation)→MIR→Codegen (zext/trunc/fptosi/sitofp/ptrtoint/inttoptr)→Interpreter |
| 8.5 | **Standard library tối thiểu** ✅ | ★★★ | `ivy::print`/`ivy::println` (type-aware, thay printf — interpreter builtin + codegen extern C). `new`/`delete` trong interpreter (cell-based). Auto-include `lib/` cho `#include <ivy.h>`. `ivy::string`/`ivy::vector<T>` planned (cần thêm language features: inline, const methods, template in header) |
| 8.6 | **`#error` / `#warning` / `#line`** ✅ | ★ | Preprocessor: `#error message` (fatal diagnostic, abort), `#warning message` (non-fatal warning), `#line N "file"` (remap line+file for diagnostics). `Diagnostic` struct thêm `isWarning` field. `emitToken()` áp dụng `lineOffset_` cho token line numbers. `main.cpp` dùng `diagFile` (từ `pp.lineFile()`) cho tất cả stage diagnostics |
| 8.7 | **Stringify `#` + paste `##`** ✅ | ★★ | Macro metaprogramming: `#param` stringifies raw (unexpanded) argument tokens (escape quotes/backslashes). `a ## b` pastes two tokens (re-lex to determine kind). Arguments pre-expanded trước normal substitution (XSTR pattern). `TokenKind::HashHash` added to lexer. `substituteBody()` handles #/##/normal substitution trong 1 pass. `ArgInfo{raw, expanded}` cho argument tracking |
| 8.8 | **Wide/UTF string prefixes** ✅ | ★ | Lexer: `isStringPrefix()` helper checks `L`/`u`/`U`/`u8`. `lexToken()` dispatches prefixed strings/chars to `lexStringLiteral(start,...)`/`lexCharLiteral(start,...)` with `start` including prefix. `lexStringLiteral`/`lexCharLiteral` signatures changed to accept `start` parameter. Codegen: `decodeStringTyped()` strips prefix and returns element width (1=i8 plain/u8, 2=i16 L/u, 4=i32 U). `StringEntry{bytes,width}` struct tracks width. `emitStringConstants()` emits `[N x i8]`/`[N x i16]`/`[N x i32]`. `decodeChar()` strips `L`/`u`/`U` prefix. mir_builder `decodeString`/`decodeChar` also strip prefixes for interpreter. Dedup key includes width |
...
| 9.2 | **Modules** ✅ | ★★★★ | `export module`, `import` — binary module interface (.ivm) thay #include. `.ivm` text format tương tự Clang `.pcm`. Flow: pre-scan tokens cho `import`, load `.ivm` trước parse, `predeclareModuleImports()` register struct/enum/alias names, merge declarations sau parse. Codegen emit `declare` cho imported functions với mangled name. `import cpp <header>` warning (dùng `#include` thay). `import`/`module` keywords added vào lexer. String interning dùng `std::deque<string>` (deque không invalidate ref khi push_back). Test: `test_module_math.ivy` → `math.ivm` + obj; `test_module_main.ivy --run` → struct fields, alias, func declare all work |
| 9.3 | **Designated initializers** ✅ | ★ | `{.field = 42}` — parse trong InitList, map field name → offset, validate thứ tự. AST `InitList.designators` vector song song với `elements`. Parser parse `.field = value` trong braced init. HIR `buildStructInit()` resolve designators → field indices, reorder thành positional, apply C++20 rules (positional continues after designator+1, duplicate detection, missing field = zero-init/default). Codegen unchanged (HIR output is positional). Interpreter unchanged (MIR is positional). Test: out-of-order, partial, mixed, empty, bool, regression all pass |
| 9.4 | **User-defined literals** (bỏ qua) | ★★ | `operator""_km(long long)` — lexer gộp suffix, HIR lookup UDL operator |
| 9.5 | **Coroutines** (bỏ qua) | ★★★★+ | `co_await`/`co_return`/`co_yield` — cần transform CFG phức tạp. Có thể KHÔNG làm (không thuộc triết lý subset) |
| 9.6 | **`std::move` + rvalue ref `T&&`** (bỏ qua) | ★★★ | Move semantics cho ownership tracking: move-out state trong MIR, use-after-move error. Nền cho smart pointer `ivy::unique_ptr<T>` |
